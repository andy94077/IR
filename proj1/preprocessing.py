...
def preprocessing(model_dir, document_dir):
    ...
    row, col, value = [], [], []
    idf = np.zeros(VOCAB_ALL_SIZE + BIGRAM_SIZE, np.float32)
    ...
    return idx2word, word2idx, np.array(row, np.int32), np.array(col, np.int32), np.array(value, np.float32), doc_len, idf#csr_matrix((value, (row, col)), shape=(FILE_NUM, len(idx2word)))
...
if __name__ == '__main__':
    ...
    if not os.path.exists(os.path.join('preprocessed', 'idx2word.pickle')) or not os.path.exists(os.path.join('preprocessed', 'word2idx.pickle')) or not os.path.exists(os.path.join('preprocessed', 'row.npy')) or not os.path.exists(os.path.join('preprocessed', 'col.npy')) or not os.path.exists(os.path.join('preprocessed', 'value.npy')) or not os.path.exists(os.path.join('preprocessed', 'doc_len.npy')) or not os.path.exists(os.path.join('preprocessed', 'idf.npy')): #os.path.exists(os.path.join('preprocessed', 'raw_tf.npz')):
        ...
        #raw_tf = load_npz(os.path.join('preprocessed', 'raw_tf.npz'))
