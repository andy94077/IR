...
def bce_generator(positiveX_pair, negativeX_pair, batch_size=128, epochs=None, positive_negative_ratio=1, seed=None):
    def concat(tup):
        return tf.concat(tup, axis=0)
    
    ds_positive = tf.data.Dataset.from_tensor_slices(positiveX_pair)
    ds_negative = tf.data.Dataset.from_tensor_slices(negativeX_pair).shuffle(int(positive_negative_ratio * positiveX_pair.shape[0]), seed=seed, reshuffle_each_iteration=True).take(int(positive_negative_ratio * positiveX_pair.shape[0]))
...
def bpr_generator(positiveX_pair, negativeX, batch_size=128, epochs=None, positive_negative_ratio=1, seed=None):
    ...
    for pos in ds_positive:
        pos = np.repeat(pos.numpy()[:, np.newaxis], positive_negative_ratio, axis=1) # [batch, positive_negative_ratio, 2]
        neg = np.stack([np.random.choice(negativeX[u], size=(positive_negative_ratio, 1)) for u in pos[:, 0, 0]])
        X = np.concatenate([pos, neg], axis=-1).reshape(-1, 3)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-r', '--positive-negative-ratio', type=int, default=1)
    ...
    if training:
        ...
        batch_size = 256
        print('start fitting')
        model.fit(generator(positive, negative, batch_size=batch_size, positive_negative_ratio=positive_negative_ratio), validation_data=generator(valid_positive, valid_negative, batch_size=batch_size, positive_negative_ratio=positive_negative_ratio), epochs=300, steps_per_epoch=positive.shape[0] // batch_size, validation_steps=valid_positive.shape[0] // batch_size, callbacks=[checkpoint, reduce_lr])
    else:
    ...
    if test:
        ...
        print(f'\033[32;1mTraining score: {model.evaluate(generator(positive, negative, epochs=1, batch_size=512, positive_negative_ratio=positive_negative_ratio), verbose=0)}\033[0m')
        print(f'\033[32;1mValidaiton score: {model.evaluate(generator(valid_positive, valid_negative, epochs=1, batch_size=512, positive_negative_ratio=positive_negative_ratio), verbose=0)}\033[0m')
